...
export default {
    ...
    computed: {
        ...
        totalAmount(): number {
            return this.cart.reduce((total, product) => (product.selected ? total + product.price * product.quantity : total),0);
        },
    },
    methods: {
        submit () {
        ...
        async deleteFromCart() {
            try {
                const db = getFirestore();
                const catalogDocRef = doc(db, 'cart', this.currentUser.uid, 'cartUser', this.deletingProductID);
        ...
        async updateCart(product: Cart) {
            try {
                const db = getFirestore();
                const cartDocRef = doc(db, 'cart', this.currentUser.uid, 'cartUser', product.productID);
