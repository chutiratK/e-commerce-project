    ...
    <v-dialog class="phone-login" v-model="showPhoneModal" max-width="400px">
      <v-card style="background-color: grey">
        ...
        <v-card-text>
          ...
          <center>
            <div v-if="warningPhone" class="warningPhone">
              Please enter phone number.
            </div>
            <div v-else-if="invalidPhone" class="invalidPhone">
  ...
  methods: {
    ...
    async sendOTP() {
      ...
      if (!this.phoneNumber) {
        ...
        try {
          const isPhoneNumberRegistered = await this.checkPhoneNumberRegistered(
            this.phoneNumber
          );
